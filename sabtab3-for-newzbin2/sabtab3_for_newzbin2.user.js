// Add SABnzbd functionality to Newzbin2
// To install, you need Greasemonkey: http://greasemonkey.mozdev.org/

// Version 3.0.1
//
// Changes from v3.0
//		Fixed the plugin, will now work with Newzbin2
// Changes from v2.1.1
...
// --------------------------------------------------------------------
// ==UserScript==
// @name          SABTab3 for Newzbin2
// @namespace     Yarble's scripts
// @description   Adds SABnzbdplus UI to Newzbin's main tab menu
// @include       http://www.newzbin.com/*
// @include       https://www.newzbin.com/*

// IMPORTANT! Requires SABnzbdplus version 0.5.0 or higher. 
...
var SABAuthString =GM_getValue("SABAuthString","");
...
(function (){	
    ...
    for (var i = 0; i < inlineCheckers.snapshotLength; i++) {
		...
		if(markDownloaded){	  // Get SAB History, call function to mark downloaded lists
			GM_xmlhttpRequest({
				method: 'GET',
				url: SABLocation +'/sabnzbd/rss?mode=history' + SABAuthString ,
				headers: {
				...
				onload: function(responseDetails) {
					...
					if (responseDetails.status == 200){				
						if(responseDetails.responseText.indexOf('{"status":false,"error":"Missing authentication"}')!= -1) return SABLogin();
		...
		if(markQueued){  // Get SAB Queue, call function to grey out matches
			GM_xmlhttpRequest({
				method: 'GET',
				url: SABLocation + '/sabnzbd/api?mode=qstatus&output=json' + (useAPIKey?('&apikey=' + APIKey):'') + SABAuthString,
				headers: {
					'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
					'Accept': 'application/atom+xml,application/xml,text/xml,text/html',
				},
...
// Create SAB tab 
	var mainMenu = document.evaluate(
	    	'//DIV[@class="bmenu"]/UL',
    	document, null,XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,null);
    	
    	if(mainMenu.snapshotLength>0) {
    	mainMenu = mainMenu.snapshotItem(0);
	}
...
// Login to SAB
function SABLogin (){
		//SAB authorization failed, clear any u/p and prompt for new
		clearSABLogin();
...
function SetSABLogin(){
	SABAuthString = (document.getElementById("SABuname").value != "" && document.getElementById("SABpwd").value != "")? '&ma_username=' + document.getElementById("SABuname").value + '&ma_password=' + document.getElementById("SABpwd").value:"";
	if(document.getElementById("saveSABuname").checked){
		GM_setValue("SABAuthString", SABAuthString);
	}
	else{
		GM_setValue("SABAuthString", "");
	}
...
function SABAdder (postNumber, argAddType) {
	...
	if (postNumber) {
		GM_xmlhttpRequest({
			method: 'GET',
			url: SABLocation + '/sabnzbd/api?mode=addid&name=' + postNumber +'&pp=' + addType + (useAPIKey?('&apikey=' + APIKey):'') + SABAuthString,
			headers: {
				'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
				'Accept': 'application/atom+xml,application/xml,text/xml,text/html',
			},
...
// Gets SAB status data and updates SAB Status Tab with results
function GetSABData(SABPage){
	  ...
	  if (typeof SABPage == 'string') {
	  	MODE='api?mode=' + SABPage + (useAPIKey?('&apikey=' + APIKey):'');
			...
			GM_xmlhttpRequest({
			method: 'GET',
			url: SABLocation +'/sabnzbd/' + MODE +  SABAuthString,
			headers: {
				'User-agent': 'Mozilla/4.0 (compatible) Greasemonkey',
				'Accept': 'application/atom+xml,application/xml,text/xml,text/html',
			},
